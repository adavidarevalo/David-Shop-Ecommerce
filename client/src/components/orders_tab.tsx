import { Alert, AlertDescription, AlertIcon, AlertTitle, Box, Button, Spinner, Stack, Table, Text, TableContainer, Tbody, Td, Th, Tr, Wrap, useDisclosure, useToast, Flex } from '@chakra-ui/react'
...
export default function OrdersTab() {
  ...
  const onSetToDelivered = (order: any) => {
    dispatch(resetErrorAndRemoval() as any)
    dispatch(setDelivered(order._id) as any)
  }
  ...
  return (
    <Box>
            ...
            <Tbody>
              {orders && orders.map(order => (
                <Tr key={order._id}>
                  ...
                  <Td>{order.email}</Td>
                  <Td>
                    <Text><i>Address: </i>{order.setShippingAddress.address}</Text>
                    <Text><i>City: </i>{order.setShippingAddress.postalCode} {order.setShippingAddress.city}</Text>
                    <Text><i>Country: </i>{order.setShippingAddress.country}</Text>
                  </Td>
                  <Td>
                    {order.orderItems.map((item: any) => (
                      <Text key={item.name}>{item.qty} x {item.name}</Text>
                    ))}
                  </Td>
                  ...
                  <Td>
                    <Flex direction={"column"}>
                      <Button variant={"outline"} onClick={() => openDeleteConfirmBox(order)}><DeleteIcon mr={"5px"} /> Remove Order</Button>
                      {!order.isDelivered && (
      ...
      <ConfirmRemovalAlert isOpen={isOpen} onClose={onClose} cancelRef={cancelRef} itemToDelete={orderToDelete} deleteAction={setOrderToDelete} />
      </Box>
  )
}
